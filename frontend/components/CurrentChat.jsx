...
const Message = (props) => {
  ...
  return (
    <Flex px={8} py={8} gap={4} bgColor={role === "assistant" ? "gray.600" : undefined}>
      {icon}
      <Flex flex="1">
        <Text color="white">
          {content}
    ...
    <Flex direction="column" flex="1" overflowY="auto" position="relative" ref={scrollRef} onScroll={onScroll}>
      ...
      <Box position="sticky" marginTop="auto" bottom={0} bgColor="gray.700">
        <Flex direction="column" py={2} alignItems="center" gap={3}>
          <ChatInput />
          <Text fontSize="xs" color="gray.500">This is a ChatGPT clone built in 30 minutes using <Link color="white" href="https://gadget.dev" isExternal>gadget.dev</Link> with modifications by me (Asad)</Text>
        </Flex>
      </Box>
    </Flex>
  )
